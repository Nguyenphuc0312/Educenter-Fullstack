...
[Authorize]
public sealed class TuitionInvoicesController(IInvoiceService service, IConfiguration configuration) : ControllerBase
{
    ...
    public async Task<IActionResult> GetById(Guid id, CancellationToken ct)
    {
        ...
        if (User.IsInRole("Student") && invoice.StudentId != GetReferenceId()) return Forbid();
        return Ok(ApiResponse<TuitionInvoiceResponse>.Ok(invoice));
    }
    ...
    [HttpGet("by-class/{classId:guid}"), Authorize(Roles = "Admin")] public async Task<IActionResult> ByClass(Guid classId, CancellationToken ct) => Ok(ApiResponse<IReadOnlyList<TuitionInvoiceResponse>>.Ok(await service.ByClassAsync(classId, ct)));
    [HttpGet("debts"), Authorize(Roles = "Admin")] public async Task<IActionResult> Debts(CancellationToken ct) => Ok(ApiResponse<IReadOnlyList<TuitionInvoiceResponse>>.Ok(await service.DebtsAsync(ct)));
    ...
    [HttpPost("from-enrollment"), AllowAnonymous]
    public async Task<IActionResult> CreateFromEnrollment(CreateInvoiceFromEnrollmentRequest request, CancellationToken ct)
    {
        if (!IsValidInternalRequest()) return Forbid();
        return Ok(ApiResponse<TuitionInvoiceResponse>.Ok(await service.CreateFromEnrollmentAsync(request, ct), "Invoice synced from enrollment"));
    }
    ...
    [HttpPut("mark-overdue-due"), Authorize(Roles = "Admin")]
    public async Task<IActionResult> MarkOverdueDue(CancellationToken ct) => Ok(ApiResponse<object>.Ok(new { updated = await service.MarkOverdueDueInvoicesAsync(ct) }, "Due invoices marked overdue"));
    ...
    {
        var expected = configuration["InternalService:SecretKey"];
        if (string.IsNullOrWhiteSpace(expected)) return false;
        return Request.Headers.TryGetValue("X-EduCenter-Internal-Key", out var actual) && actual == expected;
    }
}
