...
namespace PaymentReportService.Extensions;
...
public static class ServiceCollectionExtensions
{
    public static IServiceCollection AddEduCenterApi(this IServiceCollection services, IConfiguration configuration, string serviceName)
    {
        ...
        services.Configure<ApiBehaviorOptions>(options =>
        {
            ...
            {
                var errors = context.ModelState.Where(x => x.Value?.Errors.Count > 0).SelectMany(x => x.Value!.Errors.Select(e => new ApiError(x.Key, e.ErrorMessage))).ToList();
        ...
        services.AddCors(options => options.AddPolicy("EduCenterCors", policy => policy.WithOrigins("http://localhost:5173", "http://localhost:3000", "http://103.72.96.117").AllowAnyHeader().AllowAnyMethod()));
        services.AddEndpointsApiExplorer();
        services.AddSwaggerGen(options =>
        {
            options.SwaggerDoc("v1", new OpenApiInfo { Title = serviceName, Version = "v1", Description = "EduCenter microservice API" });
