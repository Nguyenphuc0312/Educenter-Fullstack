...
public sealed class EnrollmentsController(IEnrollmentService service) : ControllerBase
{
    ...
    [HttpPut("{id:guid}/start-study"), Authorize(Roles = "Admin")] public async Task<IActionResult> StartStudy(Guid id, CancellationToken ct) => Ok(ApiResponse<EnrollmentResponse>.Ok(await service.SetStatusAsync(id, EnrollmentStatus.Studying, ct)));
